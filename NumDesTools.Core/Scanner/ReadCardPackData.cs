...
/// <summary>
/// 一次性诊断脚本：读取活动照片收集卡包信息和活动服务端表。
/// 原为 top-level statements，改为静态方法以避免占用程序入口点。
/// 调用：ReadCardPackData.Run()
/// </summary>
internal static class ReadCardPackData
{
    public static void Run()
    {
        ...
        try
        {
            ...
            for (int row = 2; row <= sheet.Dimension?.Rows; row++)
            {
                ...
                if (
                    idCell.Value != null
                    && int.TryParse(idCell.Value.ToString(), out int id)
            ...
            Console.WriteLine(
                $"找到的卡包ID: {string.Join(", ", targetRows.Keys.OrderBy(x => x))}\n"
            );
        ...
        try
        {
            ...
            var sheetNames = package.Workbook.Worksheets.Select(s => s.Name).ToList();
            Console.WriteLine(
                $"可用Sheet({sheetNames.Count}个): {string.Join(", ", sheetNames)}\n"
            );
            ...
            Console.WriteLine(
                $"维度: {mainSheet.Dimension?.Rows} rows x {mainSheet.Dimension?.Columns} cols\n"
            );
            ...
            Console.WriteLine(
                $"字段列表({fieldList.Count}个): {string.Join(", ", fieldList.Take(10))}"
            );
            ...
            var cardPackIds = new HashSet<int>
            {
                5600011,
                5600012,
                5600013,
            ...
            for (int row = 2; row <= mainSheet.Dimension?.Rows; row++)
            {
                ...
                if (
                    actValue != null
                    && int.TryParse(actValue.ToString(), out int actId)
                    && targetActivityIds.Contains(actId)
                )
                {
                    ...
                    for (int col = 1; col <= mainSheet.Dimension?.Columns; col++)
                    {
                        ...
                        if (
                            cellValue != null
                            && cardPackIds.Any(id => cellValue.ToString()!.Contains(id.ToString()))
                        )
                        {
                            hasCardPack = true;
                            break;
