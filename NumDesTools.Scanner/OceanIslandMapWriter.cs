...
public static class OceanIslandMapWriter
{
    ...
    public static void Run(string outputPath, string dataPath = DefaultDataPath)
    {
        ...
        BuildMapSheet(pkg, "总览", data, data.MinR, data.MaxR, data.MinC, data.MaxC);
    ...
    private static OceanData LoadData(string dataPath)
    {
        ...
        var clouds = JsonSerializer.Deserialize<JsonElement[]>(f)
        ...
        int minR, int maxR, int minC, int maxC)
    {
        ...
        for (int ri = 0; ri < nRows; ri++)
        {
            ...
            {
                int gameC = minC + ci;
                var item  = data.AllTiles.GetValueOrDefault($"{gameR}-{gameC}", "");
