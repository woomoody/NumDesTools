...
public static class FeishuMcpClient
{
    ...
    public static Task<JToken> ListCommentsAsync(string workItemId)
        => CallAsync("list_workitem_comments", new { project_key = ProjectKey, work_item_id = workItemId });
    ...
    public static Task<JToken> UpdateTextField(
        string workItemId, string workItemTypeKey, string fieldKey, string text)
        => CallAsync("update_field", new
        {
            ...
            fields = new[]
            {
                new { field_key = fieldKey, field_value = text }
            }
        });
    ...
    /// </summary>
    public static async Task<string> GetCurrentFieldValueAsync(
        string workItemId, string fieldKey)
    {
        try
        {
            var result = await CallAsync("get_workitem_brief", new
            {
                project_key  = ProjectKey,
                work_item_id = workItemId,
                fields       = new[] { fieldKey }
