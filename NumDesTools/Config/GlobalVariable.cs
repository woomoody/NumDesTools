...
namespace NumDesTools.Config
{
    public class GlobalVariable
    {
        ...
        // 默认列表配置
        public readonly List<string> _defaultNormaKeyList = new()
        {
            ",,",
            "[,",
            ",]",
        ...
        public readonly List<CoupleKey> _defaultCoupleKeyList = new()
        {
            new CoupleKey("[", "]"),
            new CoupleKey("{", "}")
        };
        ...
        public void ReadOrCreate()
        {
            if (File.Exists(_filePath))
            {
                ...
                // 处理列表配置
                if (fileValues.ContainsKey("NormaKeyList") && fileValues["NormaKeyList"] is List<object> normaKeyList)
                {
                    _configData.NormaKeyList = normaKeyList.ConvertAll(x => x.ToString());
                }

                if (fileValues.ContainsKey("SpecialKeyList") && fileValues["SpecialKeyList"] is List<object> specialKeyList)
                {
                    _configData.SpecialKeyList = specialKeyList.ConvertAll(x => x.ToString());
                }

                if (fileValues.ContainsKey("CoupleKeyList") && fileValues["CoupleKeyList"] is List<object> coupleKeyList)
                {
                    _configData.CoupleKeyList = coupleKeyList.ConvertAll(x =>
                    {
                        var dict = JsonConvert.DeserializeObject<Dictionary<string, string>>(x.ToString() ?? string.Empty);
                        return new CoupleKey(dict["Left"], dict["Right"]);
                    });
                }
        ...
        private void MergeWithDefaults()
        {
            ...
            _configData.SpecialKeyList = MergeLists(_configData.SpecialKeyList, _defaultSpecialKeyList);
            _configData.CoupleKeyList = MergeLists(_configData.CoupleKeyList, _defaultCoupleKeyList);
        }
        ...
        {
            if (_configData.Value.ContainsKey(key))
            {
                _configData.Value[key] = value;
            }
            else
            {
                _configData.Value.Add(key, value);
            }
