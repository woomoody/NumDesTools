    ...
    public static class LuaCodeGenerator
{
    ...
    {
        "Tables.ChangeRewardGroupData", "Tables.RedPoint", "Tables.BlockedTile", "Tables.ItemAdsorb", 
        "Tables.PropsGiftBagTrigger", "Tables.BpBuildData", "Tables.BpChestData", "Tables.BpCreateData",
        "Tables.BpMergeData", "Tables.BpOrderData", "Tables.BpCollectData", 
        "Tables.EventTriggerData","Tables.EventTriggerLteData","Tables.EventTriggerMustData","Tables.EventTriggerZooData","Tables.ChangeChainData",
        "Tables.UserHierarchyCountry","Tables.UserHierarchyAf","Tables.UserHierarchy","Tables.ItemIdMapping","Tables.ActivityMineBuffDropIdMapping",
        "Tables.RedPointIdMapping","Tables.ScoreIdMapping","Tables.IconIdMapping","Tables.UserGroupActivity","Tables.PopsGiftLteEnergyExtraParam",
        "Tables.AdsCoolingData","Tables.GuideGroupHierarchy","Tables.ActivityClientHierarchyGroupData","Tables.DropIdMapping","Tables.PopsGiftActRewardExtraParam",
        "Tables.ActivityBalloonEffect","Tables.QAConfig", "Tables.ActivityBpRewardChangeData","Tables.GuideGroupData","Tables.FlotageSkinSceneData",
        "Tables.EventTriggerLtePortalData","Tables.KlondikeBag","Tables.ActivityPhotoIdMapping","Tables.IconIdChangeByCondition","Tables.SkinRewardConvertData",
        "Tables.HelpTargetIdMapping","Tables.ShopMarketIds","Tables.HelpTargetIdMapping","Tables.Field01Data","Tables.Field03Data",
        "Tables.US_WHAT_W","Tables.US_WHAT_H","Tables.US_WHAT_A","Tables.US_WHAT_T","Tables.US_WHAT_Label","Tables.HelpMustItemIdUI","Tables.EntryGatherInfos",
        "Tables.LteIconIdMapping_","Tables.LteElementPrefabMapping_","Tables.LteStringKeyMapping_","Tables.EventTriggerMainStoryLevelData"
    };
    ...
    public static string ToLuaTable(SheetData data, string tableName, FieldData commentField = null,bool isIgnoreCheckNullValue = false)
    {
        ...
        if (!isIgnoreCheckNullValue && !data.hadDefaultValue && !IsNotAddCheckNullValue(tableName))//该表不包含默认值处理（默认值内部会处理id空）&&不在过滤配置中
        {
    ...
    public static string ToLuaTableByKeyValue(SheetData data, string tableName, FieldData keyField,
        FieldData valueField, FieldData commentField = null, string tableDesc = "", bool isAddCheckNullCode = true)
    {
        StringBuilder text = new StringBuilder();
    ...
    public static string FieldsToLuaTable(List<FieldData> fields, string name, int startIndex = 1,
        string annotation = "")
    {
        StringBuilder text = new StringBuilder();
    ...
    private static void RowData2LuaTable(SheetData sheet, RowData row, FieldData commentField,
        List<FieldData> ignoreFields,ref StringBuilder text)
    {
        var cells = row.cells;
        //解析key
        ...
        for (int i = 0; i < sheet.fields.Count; i++)
        {
            ...
            if (field.activeDefaultValue && 
                (string.IsNullOrEmpty(cells[field.index].value) || cells[field.index].value.Equals(field.defaultValue)))
    ...
    private static void AddLuaAnnotation(StringBuilder text, SheetData data, string tableName)
    {
        ...
        for (int i = 0; i < data.fields.Count; i++)
        {
            ...
            if (field.name == SplitFieldName) continue; // 跳过分表字段
            text.AppendLine($"---@field {field.name} {GetLuaAnnotationType(field)} @{field.desc}");
        }
    ...
    private static string Cell2LuaValue(CellData cell, FieldData field)
    {
        ...
        if (field.type != FieldTypeDefine.STRING && field.type != FieldTypeDefine.STRING_ARRAY && field.type != FieldTypeDefine.LUA_TABLE &&
    ...
    private static string GetLuaValueByType(string v, int t)
    {
        switch (t)
        {
            ...
            case FieldTypeDefine.STRING:
                ...
                return $"\"{v}\"";
            case FieldTypeDefine.STRING_ARRAY:
                if (string.IsNullOrEmpty(v) || v.Equals("[]")) return "{}";
                if (v.IndexOf('"') < 0)
                {
                    string str = v;
                    if (str.IndexOf('[') < 0) 
            ...
            case FieldTypeDefine.REWARD:
                ...
                {
                    if(!ValidReward(text)) throw new Exception($"奖励格式不正确! {v}");
                    return $"__reward({text})";
                }
                if (t == FieldTypeDefine.REWARD_ARRAY)
                {
                    if(!ValidRewardArray(text)) throw new Exception($"奖励格式不正确! {v}");
                    return $"__reward2({text})";
                }
                return text;
            case FieldTypeDefine.LUA_TABLE:
                if (string.IsNullOrEmpty(v)) return "nil";
                return v;
        }
        return string.Empty;
    }
    ...
    /// </summary>
    private static void CreateDataSegmentMode1(SheetData data, string tableName,FieldData commentField,
        ref StringBuilder text)
    {
        string className = "Excel." + tableName.Replace("Tables.", "");
        if (data.fields[0].type != FieldTypeDefine.STRING)
        {
            text.AppendLine($"---@type table<{GetLuaAnnotationType(data.fields[0])},{className}>");
        }
        text.AppendLine($"{tableName} = {{");
        //行数据转成Table数组
        for (int i = 0; i < data.rows.Count; i++)
    ...
    /// </summary>
    private static void CreateDataSegmentMode2(SheetData data, string tableName,List<FieldData> ignoreFields,
        string mainTableName,ref StringBuilder text,bool isIgnoreCheckNullValue)
    {
        if (string.IsNullOrEmpty(mainTableName)) // 子表不生成默认值，用主表
        {
            ...
            for (int i = 0; i < data.fields.Count; i++)
            {
                ...
                {
                    text.AppendLine($"        {data.fields[i].name} = {Field2LuaValue(data.fields[i])},"); // 逐行定义默认值
                }
            }
            text.AppendLine("    },");
            text.AppendLine("}");
        ...
        text.AppendLine($"{tableName} = {{}}");
        text.AppendLine(string.IsNullOrEmpty(mainTableName)
    ...
    public static Dictionary<string,string> ToLuaTableSplitMode(SheetData data, string tableName,bool isIgnoreCheckNullValu)
    {
        #region 查找分组字段
        ...
        {
            if (int.TryParse(Cell2LuaValue(data.rows[i].cells[splitField.index],splitField),out int subId))
            {
        ...
        //数据内容
        CreateDataSegment_SplitMode_MainData(data,tableName,splitField,ref text,isIgnoreCheckNullValu);
        ...
        foreach (KeyValuePair<int,SheetData> valuePair in subDataCache)
        {
            ...
            //子表不创建表头，参数定义lua注释
            //数据内容
            CreateDataSegmentMode2(valuePair.Value,subTabName,subTableIgnoreFields, tableName, ref text,isIgnoreCheckNullValu);
    ...
    /// 创建数据段 - 分表模式 - 仅生成总表数据
    /// </summary>
    private static void CreateDataSegment_SplitMode_MainData(SheetData data, string tableName,FieldData subfields,ref StringBuilder text,bool isIgnoreCheckNullValu)
    {
        text.AppendLine("\n---数据单元元表");
        text.AppendLine("local dataCellMetaTable = {");
        text.AppendLine("    __index = {");
        for (int i = 0; i < data.fields.Count; i++)
        {
            if (data.fields[i].name == SplitFieldName) continue; // 子表列跳过默认赋值
            if (data.fields[i].activeDefaultValue) // 激活默认值
            {
                text.AppendLine($"        {data.fields[i].name} = {Field2LuaValue(data.fields[i])},"); // 逐行定义默认值
            }
        ...
        if (data.fields[0].type != FieldTypeDefine.STRING)
            text.AppendLine($"---@type table<{GetLuaAnnotationType(data.fields[0])},{className}>");
        text.AppendLine($"{tableName} = {{_subTableCache = {{}}, _dataCellMetaTable = dataCellMetaTable}}");
        text.AppendLine($"Tables.SetSubTableMetatable({tableName},data,\"{data.name}\",{(!IsNotAddCheckNullValue(tableName) && !isIgnoreCheckNullValu).ToString().ToLower()})");
    }
    ...
    private static void RowData2LuaTable_SplitMode_MainData(SheetData sheet, RowData row,FieldData subfields, ref StringBuilder text)
    {
        var cells = row.cells;
        //解析key
        string key = Cell2LuaValue(cells[0], sheet.fields[0]);
