...
namespace NumDesTools.ExcelToLua
{
    ...
    public class ExcelReader
    {
        ...
        {
            if (CheckStringChinese(sheet.SheetName))
                return false;
            if (sheet.GetRow(0) == null)
                return false;
            if (sheet.GetRow(0).Cells.Count == 0)
                return false;
            if (isClient && sheet.SheetName.StartsWith("s_"))
                return false;
            if (!isClient && sheet.SheetName.StartsWith("c_"))
                return false;
        ...
        public static void ReadSheetFields(ISheet sheet, ref SheetData data)
        {
            ...
            for (int j = data.startCol; j < colCount; j++)
            {
                ...
                if (fieldName.Trim().Length == 0 || fieldName.Trim().Length == 0)
                    continue;
        ...
        /// <returns></returns>
        public static List<SheetData> Read(
            string excelfile,
            int startRow = 0,
            int startCol = 0,
            bool singleSheetMode = false,
            bool ignoreRowData = false,
            bool isLog = true
        )
        {
            ...
            for (int i = 0; i < sheets.Count; i++)
            {
                ...
                if (!ignoreRowData)
                    ReadSheetRows(sheet, ref data);
                string filename = singleSheetMode
                    ? Path.GetFileNameWithoutExtension(excelfile)
                    : sheet.SheetName;
                data.name = filename;
                list.Add(data);
                if (singleSheetMode)
