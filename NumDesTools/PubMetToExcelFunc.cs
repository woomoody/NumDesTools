...
public class PubMetToExcelFunc
{
        ...
        if (data.TryGetValue(keyName, out var valueList))
        {
            ...
            else
            {
                var tips = MessageBox.Show(
                    "字段未关联或没有表格索引,是否打开字段表格编辑？是：打开表编辑；否：模糊查找相似度最高的关联表",
                    "确认",
                ...
                else
                {
                    ...
                    // 查找最相似的键并返回对应的值
                    string blurResult = FindClosestMatch(blurData, keyCell.Value.ToString());
    ...
    private static string FindClosestMatch(List<object> listOfObjects, string input)
    {
        ...
        // 返回最相似键对应的值
        return listOfLists.FirstOrDefault(list => list[0] == closestKey)?[1];
    }
