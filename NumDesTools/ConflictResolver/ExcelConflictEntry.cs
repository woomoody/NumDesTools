...
public static class ExcelConflictEntry
{
    ...
    public static void OpenGitConflict()
    {
        ...
        while (true)
        {
            ...
            if (!applied) continue; // 用户点了取消，返回文件选择
        }
    ...
    public static void OpenGitHistory()
    {
        ...
        var allCommits  = new List<(string sha, string shortSha, string date, string author, string message)>();
        int loadedCount = 0;
        ...
        {
            Text      = "与当前工作区对比",
            Height    = 32, Width = 160, Left = 10, Top = 6,
            FlatStyle = FlatStyle.Flat,
            BackColor = System.Drawing.Color.FromArgb(0, 122, 204),
            ForeColor = System.Drawing.Color.White,
        ...
        {
            Text      = "与另一历史版本对比",
            Height    = 32, Width = 168, Left = 180, Top = 6,
            FlatStyle = FlatStyle.Flat,
        ...
        // 追加一批到 ListBox
        void AppendBatch(IEnumerable<(string sha, string shortSha, string date, string author, string message)> batch)
        {
            lb.BeginUpdate();
        ...
        void LoadNextPage()
        {
            ...
            System.Threading.ThreadPool.QueueUserWorkItem(_ =>
            {
                ...
                if (!picker.IsHandleCreated || picker.IsDisposed) { isLoading = false; return; }
                picker.BeginInvoke((System.Action)(() =>
                {
        ...
        {
            if (e.KeyCode == Keys.Escape) picker.Close();
            if (e.KeyCode == Keys.Tab) { lb.Focus(); e.Handled = true; }
        };
        ...
        while (true)
        {
            ...
            if (selectedIdx < 0 || selectedIdx >= allCommits.Count) continue;
            var selectedSha = allCommits[selectedIdx].sha;

            if (mode == "working")
            {
                var histPath = Path.Combine(tmpDir, $"hist_{allCommits[selectedIdx].shortSha}_{fileName}");
                try { GitShowBySha(gitRoot, selectedSha, relativePath, histPath); }
                catch (Exception ex)
                {
                    MessageBox.Show($"提取历史版本失败：{ex.Message}", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    continue;
                }
            ...
            {
                // 第二个版本选择窗口
                var allCommits2  = new List<(string sha, string shortSha, string date, string author, string message)>(allCommits);
                ...
                var statusLabel2 = new System.Windows.Forms.Label
                {
                    ...
                    Text = hasMore2 ? $"已加载 {loadedCount2} 条，滚动到底加载更多" : $"共 {loadedCount2} 条，已全部加载",
                    ForeColor = System.Drawing.Color.FromArgb(130, 130, 130),
                    Font = new Font("微软雅黑", 8f),
                    TextAlign = System.Drawing.ContentAlignment.MiddleLeft,
                    Padding = new Padding(8, 0, 0, 0),
                ...
                void LoadNextPage2()
                {
                    ...
                    System.Threading.ThreadPool.QueueUserWorkItem(_ =>
                    {
                        ...
                        try { page2 = ReadGitLogForFile(gitRoot, relativePath, skip2, PageSize); }
                        catch { page2 = []; }
                        if (!picker2.IsHandleCreated || picker2.IsDisposed) { isLoading2 = false; return; }
                        picker2.BeginInvoke((System.Action)(() =>
                        {
                ...
                {
                    if (e.KeyCode == Keys.Escape) picker2.Close();
                    if (e.KeyCode == Keys.Tab) { lb2.Focus(); e.Handled = true; }
                };

                var bottomPanel2 = new Panel { Dock = DockStyle.Bottom, Height = 44, BackColor = System.Drawing.Color.FromArgb(37, 37, 40) };
                var btnOk2 = new Button
                {
                    Text = "开始对比", Height = 32, Width = 110, Left = 10, Top = 6,
                    FlatStyle = FlatStyle.Flat,
                    BackColor = System.Drawing.Color.FromArgb(0, 122, 204),
                    ForeColor = System.Drawing.Color.White,
                    Font = new Font("微软雅黑", 9.5f, System.Drawing.FontStyle.Bold),
                ...
                if (picker2.ShowDialog() != DialogResult.OK) { picker2.Dispose(); continue; } // 取消第二个版本选择 → 回历史选择器
                var selectedIdx2 = lb2.SelectedIndex;
                picker2.Dispose();

                var sha2      = allCommits2[selectedIdx2].sha;
                var histPath  = Path.Combine(tmpDir, $"hist_{allCommits[selectedIdx].shortSha}_{fileName}");
                var histPath2 = Path.Combine(tmpDir, $"hist_{allCommits2[selectedIdx2].shortSha}_{fileName}");
                try
                {
    ...
    private static string FindGitExe()
    {
        ...
        var stGit = Path.Combine(appData, "Atlassian", "SourceTree", "git_local", "mingw32", "bin", "git.exe");
        if (File.Exists(stGit)) return stGit;
    ...
    /// </summary>
    private static List<(string sha, string shortSha, string date, string author, string message)>
        ReadGitLogForFile(string gitRoot, string relativePath, int skip = 0, int limit = 30)
    {
        // --format=<sha>|<date>|<author>|<subject>，用 | 分隔，避免空格歧义
        var args = $"log --follow --format=\"%H|%ai|%an|%s\" --skip={skip} --max-count={limit} -- \"{relativePath.Replace('/', '\\')}\"";
                                    ...
                                    string? outPath, bool autoGitAdd)
    {
        ...
        waitForm.Controls.Add(new System.Windows.Forms.Label
        {
            Text      = "正在比较文件，请稍候…",
            ForeColor = System.Drawing.Color.White,
            AutoSize  = true,
        ...
        {
            try   { diff   = ExcelConflictDiffer.Diff(oursPath, theirsPath); }
            catch (Exception ex) { diffEx = ex; }
            finally
            {
                // 用 BeginInvoke 避免：若 ShowDialog 尚未建立 Handle，Invoke 会抛异常
                try { waitForm.BeginInvoke((System.Action)waitForm.Close); }
                catch { /* 窗口已销毁，忽略 */ }
            }
        }) { IsBackground = true };
        ...
        if (diffEx != null)
        {
            MessageBox.Show($"解析文件失败：{diffEx.Message}", "错误",
                MessageBoxButtons.OK, MessageBoxIcon.Error);
            return false;
        }
