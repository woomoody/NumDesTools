...
public class RatioCaculate
{
    public static void CacCardCollect()
    {
        ...
        var collectCardGroupData = PubMetToExcel.ExcelDataToListBySelf(
            collectCardGroup,
            5,
            1,
            2,
        ...
        var collectCardRarityData = PubMetToExcel.ExcelDataToListBySelf(
            collectCardRarity,
            5,
            1,
            2,
        ...
        var collectCardScoreData = PubMetToExcel.ExcelDataToListBySelf(
            collectCardScore,
            5,
            1,
            2,
        ...
        for (var i = 0; i < countRarity; i++)
        {
            ...
            newGroupRarityCount.Add(
                (groupRarityCount[i].Item1, newRarity1, newRarity2, newRarity3)
            );
            ...
            for (var a = 0; a < randCountTotal.Item3.Count; a++)
                collectCardGroup.Cells[a + 5, cardGroupIndex + 6 + i].Value = randCountTotal.Item3[
                    a
                ];
        }
    }

    private static Tuple<double, double, List<double>> RandCount(
        dynamic groupRarityCount,
        int i,
        dynamic newGroupRarityCount,
        int maxScore,
        int score3,
        int score2,
        int score1,
    ...
    )
    {
        ...
        for (var s = 0; s < simTimes; s++)
        {
            ...
            while (
                ...
                || cardList2.Count < newGroupRarityCount[i].Item3
            ...
            )
            {
                if (currentScore >= maxScore)
                {
                    ...
                    else
                    {
                        if (cardList == cardList1)
                        {
                            ...
                            for (var k = 0; k < i + 1; k++)
                                if (
                                    cardSeed <= currentLimit1[k].Item2
                                    && cardSeed > currentLimit1[k].Item1
                                )
                                    currentCount1[k]++;
                        }
                        else if (cardList == cardList2)
                        {
                            ...
                            for (var k = 0; k < i + 1; k++)
                                if (
                                    cardSeed <= currentLimit2[k].Item2
                                    && cardSeed > currentLimit2[k].Item1
                                )
                                    currentCount2[k]++;
                        }
                        else if (cardList == cardList3)
                        {
                            ...
                            for (var k = 0; k < i + 1; k++)
                                if (
                                    cardSeed <= currentLimit3[k].Item2
                                    && cardSeed > currentLimit3[k].Item1
                                )
                                    currentCount3[k]++;
                        }
                    }
                ...
                else
                {
                    ...
                    )
                    {
                        ...
                        rairtySeed <= weight1 + weight2 + weight3
                        && rairtySeed > weight1 + weight2
                        && weight3 != 0
                    )
                    {
                ...
                for (var k = 0; k < i + 1; k++)
                    if (
                        currentCount1[k] == groupRarityCount[k].Item2
                        && currentCount2[k] == groupRarityCount[k].Item3
                        && currentCount3[k] == groupRarityCount[k].Item4
                        && otherRankCount[k] == 0
                    )
        ...
        var resultValue = new Tuple<double, double, List<double>>(
            (double)maxRankCountTotal / simTimes,
            (double)scoreGetTimes / simTimes,
            otherRankCountTotal
        );
        return resultValue;
    }
