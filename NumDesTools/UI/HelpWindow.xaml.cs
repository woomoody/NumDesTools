...
namespace NumDesTools.UI
{
    public partial class HelpWindow : MetroWindow
    {
        ...
        private static readonly List<HelpGroup> HelpData = new()
        {
            new(
                "格式检查",
                new()
...
<p class='summary'>复制当前激活工作簿的完整文件路径到剪贴板。</p>"
                    ...
                    new(
                        ...
                        @"
<h2>🗂️ 源表 / 目标路径</h2>
<p class='summary'>两个可编辑文本框，分别用于配置「源表根目录」和「目标根目录」。各功能（全局搜索等）会读取这两个路径。输入后自动保存到本地配置。</p>
<h3>使用步骤</h3>
<ol>
...
#placeholder {
    ...
    font-size: 20px;
        ...
        private static List<HelpGroup> LoadHelpDataFromJson()
        {
            try
            {
                var asm = Assembly.GetExecutingAssembly();
                var name = asm.GetManifestResourceNames()
                    .FirstOrDefault(n => n.EndsWith("help.json"));
                if (name is null)
                    return null;
                using var stream = asm.GetManifestResourceStream(name);
                using var reader = new StreamReader(stream!);
                var json = JObject.Parse(reader.ReadToEnd());
                var result = new List<HelpGroup>();
                foreach (var grpToken in json["groups"]!)
                {
                    var items = grpToken["items"]!
                        .Select(i => new HelpItem(
                            i["title"]!.ToString(),
                            i["html"]?.ToString() ?? $"<p>{i["summary"]}</p>"
                        ))
                        .ToList();
                    result.Add(new HelpGroup(grpToken["group"]!.ToString(), items));
                }
                return result;
            }
            catch
        ...
        {
            var data = LoadHelpDataFromJson() ?? HelpData;
