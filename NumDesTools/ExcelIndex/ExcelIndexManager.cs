...
internal sealed class ExcelIndexManager
{
    ...
    private readonly ConcurrentDictionary<string, byte> _pendingRebuild = new(
        ...
        return query
            .Select(h =>
                (
                    Path.Combine(
                        root,
                        (idx.Files.Count > h.FileId ? idx.Files[h.FileId] : "").Replace(
                            '/',
    ...
    /// <summary>
    /// 前缀模糊搜索（keyword* 模式）。利用 SortedKeys 二分定位，比遍历快。
    /// 返回 null 表示索引未就绪或 SortedKeys 未构建。
    /// </summary>
    public List<(string file, string sheet, int row, int col)>? TrySearchPrefix(
        string prefix,
        int colFilter = 0
    )
    {
        ...
        var hits = idx.SearchByPrefix(prefix);
        var root = _excelsRoot ?? "";
        var query = hits.AsEnumerable();
        if (colFilter > 0)
            query = query.Where(h => h.Col == colFilter);
    ...
    /// <summary>
    /// 包含模糊搜索（*keyword* 模式）。遍历 key 做 Contains，cap 500 条防海量结果。
    /// 返回 null 表示索引未就绪。
    /// </summary>
    public List<(string file, string sheet, int row, int col)>? TrySearchContains(
        string keyword,
        int colFilter = 0
    )
    {
        ...
        var hits = idx.SearchByContains(keyword, StringComparison.OrdinalIgnoreCase);
        var root = _excelsRoot ?? "";
        var query = hits.AsEnumerable();
        if (colFilter > 0)
            query = query.Where(h => h.Col == colFilter);
    ...
    {
        var hits = index.SearchByPrefix(prefix);
        var root = excelsRoot.TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar;
        var result = new List<(string, string, int, int)>();
