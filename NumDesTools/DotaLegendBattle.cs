...
{
    private static readonly object m_monitorObject = new();
...
internal class DotaLegendBattle
{
    ...
    public static int battle()
    {
        ...
        do
        {
            ...
            //else
            {
                ...
                var ts2 = sw.Elapsed;
                batime++;
                Debug.Print(ts2 + "每次计算开销:计算次数" + batime);
            }
            turn++;
    ...
    private static void DamageCaculate(dynamic def2Dam, int i, dynamic crit1Dam, dynamic atk1Dam, dynamic critMulti1Dam,
        dynamic skillDamge1Dam, dynamic hp2Dam, dynamic iDam, dynamic num1Dam, dynamic hp1Dam,
        ...
        dynamic name1Dam, dynamic name2Dam, dynamic isAB)
    {
        ...
        battleLog += name1Dam[i] + "[" + tempRole1 + "]" + "攻击" + name2Dam[iDam] + "[" + tempRole2 + "]" + "造成伤害：" +
                     Convert.ToInt32(dmg) + "\r\n";
        if (isSkillDam)
        {
            //遍历群体加血;只有使用技能时使用
    ...
    private static void tarHpDiv(int num1, dynamic def2Dam, dynamic iDam, dynamic crit1Dam, dynamic atk1Dam,
        dynamic critMulti1Dam, dynamic skillDamge1Dam)
    {
