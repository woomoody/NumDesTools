...
namespace NumDesTools.Advance
{
    internal class ExcelDataToDb
    {
        ...
        {
            UpdateSingleFile(filePath, dbPath, updateMode, new ConversionOptions());
        }

        // 新增支持配置选项的方法
        public void UpdateSingleFile(string filePath, string dbPath, UpdateMode updateMode, ConversionOptions options)
        {
            ...
            if (options.SkipHashFiles && filePath.Contains("#"))
            {
        ...
        public class ConversionOptions
        {
            ...
            /// <summary>
            /// 是否跳过包含#的文件和工作表（默认true）
            /// </summary>
            public bool SkipHashFiles { get; set; } = true;

            /// <summary>
            /// 类型推断采样行数（默认50）
            /// </summary>
            public int TypeInferenceSampleSize { get; set; } = 50;
            ...
            /// 是否验证行号映射（默认true）
            /// </summary>
            public bool VerifyRowMappings { get; set; } = true;
        }
            ...
            try
            {
                ...
                foreach (var sheetName in sheets)
                {
                    ...
                    else
                    {
                        ...
                        // 优化：直接创建新表，避免重复调用ProcessSingleExcelFile
                        CreateNewTableFromExcel(connection, filePath, sheetName, tableName);
                    }
        ...
        private void CreateNewTableFromExcel(SqliteConnection connection, string filePath, string sheetName, string tableName)
        {
            try
            {
                ...
                if (rows.Count > 0)
                {
                    ...
                    Debug.Print($"创建新表完成: {tableName}, 行数: {rows.Count}");
                }
                else
                {
                    Debug.Print($"Excel文件为空，跳过创建表: {tableName}");
                }
            }
            catch (Exception ex)
            {
                Debug.Print($"创建新表失败 {tableName}: {ex.Message}");
                throw;
            }
        }
    ...
    Dictionary<string, Type> newColumnTypes)
       ...
       string fileFullPath, string sheetName, string fileNameOnly)
        ...
        private static void InsertDataWithTypes(
            ...
            int batchSize = 5000  // 增大批次大小以提高性能
        )
        {
            if (rows.Count == 0)
                return;
        ...
        private static Dictionary<string, Type> InferColumnTypes(List<dynamic> rows)
        {
            ...
            foreach (var column in firstRow.Keys)
            {
                ...
                // 优化：采样更多行以提高类型推断准确性
                var sampleValues = rows.Take(Math.Min(50, rows.Count))
                ...
                {
                    // 优化类型推断逻辑
                    var intCount = sampleValues.Count(v => int.TryParse(v.ToString(), out _));
                    var doubleCount = sampleValues.Count(v => double.TryParse(v.ToString(), out _));
                    var dateCount = sampleValues.Count(v => DateTime.TryParse(v.ToString(), out _));

                    // 如果大部分值可以转换为整数，则推断为整数类型
                    if (intCount > sampleValues.Count * 0.8)
                        inferredType = typeof(int);
                    // 如果大部分值可以转换为浮点数，则推断为浮点数类型
                    else if (doubleCount > sampleValues.Count * 0.8)
                        inferredType = typeof(double);
                    // 如果大部分值可以转换为日期，则推断为日期类型
                    else if (dateCount > sampleValues.Count * 0.8)
